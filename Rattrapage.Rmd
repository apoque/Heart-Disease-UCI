...
<br/>
On peut ici remarquer qu'avoir une fréquence cardiaque haute basse semble être un facteur de risque de thrombose coronaire.
<br/>
...
ggplot(data,aes(x=resting_ecg,fill=angiographic_disease))+
  geom_bar()+
  geom_text(aes(label=..count..),stat='count',position=position_stack(vjust = 0.5))+
  ggtitle("Taux de diagnostic de thrombose coronaire selon l'ECG de repos")+
  theme_minimal()
...
<br/>
Il semblerait que les patients atteints de thrombose coronaire aient moins de douleure angineuse à l'effort.
Ce résultat semble contre-intuitif, mais il pourrait permettre de faire un diagnostic différentiel avec une d'autres maladies cardiovasculaires.
<br/>
...
<br/>
Il semblerai qu'avoir une pente de son segment ST lors d'un effort (qu'elle soit positive ou négative) semble être associé avec une thrombose coronaire.
<br/>
```{r, warning=FALSE, message=FALSE, echo=FALSE}
...
<br/>
Il semblerai donc qu'en demandant au patient de faire une épreuve de test on puisse déjà avoir une bonne idée de la présence ou non de thrombose coronaire.
...
data %>% filter(ex_angina == FALSE & st_depression <= 2 & tmpBis$chest_pain == "symptomatic") -> oui
data %>% filter(ex_angina == TRUE | st_depression > 2 | tmpBis$chest_pain == "asymptomatic") -> non
