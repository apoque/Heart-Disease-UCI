...
<br/>  14. Diagnostic de thrombose coronaire.
<br/b> 15. Classe d'age, par tranches de 10 ans.

On va donc s'intéresser aux caractéristiques des différents groupes de patients, et chercher à comprendre comment ces caractéristiques influent sur l'apparition des maladies cardiovasculaires.
...
data$age_class = discretize(data$age, method = "fixed", breaks = 7, categories = c(20,30,40,50,60,70,80))
...
colnames(data)[colnames(data)=="trestbps"] <- "resting_bp"
...
Le jeu de donnees comporte `r nrow(data)` patients, dont `r data %>% filter(data$sex == "M") %>% nrow` hommes et `r data %>% filter(data$sex == "F") %>% nrow` femmes.
```{r}
ggplot(data = data)+
...
```
<br/>
On remarque donc qu'on a 2 fois plus d'hommes que de femmes. Afin d'avoir des résultats interprétables il serait donc intéressant de travailler à partir de ratios ou de pourcentage pour chaque sexe. Enfin, plus de 90% des patients ont entre 30 et 70 ans. Il sera donc compliqué d'étendre nos résultats au delà de cette tranche d'âge.
...
```{r}
ggplot(data = data)+
  geom_bar(aes(x= age_class, fill = angiographic_disease), position = "dodge")

ggplot(data = data)+
  geom_jitter(aes(x=age_class, y = sex, color = angiographic_disease))

Fcardiac <- 100*as.numeric(data %>% filter(angiographic_disease == TRUE,
                                       sex == "F") %>% 
                         summarise(angiographic_disease = sum(angiographic_disease)))/96

Hcardiac <- 100*as.numeric(data %>% filter(angiographic_disease == TRUE,
                                       sex == "M") %>% 
                         summarise(angiographic_disease = sum(angiographic_disease)))/96
```

Plus forte prévalence des maladies cardiaques chez les hommes puisqu'on est 96.875% alors que chez la femme on est à 75%.
La classe d'âge la plus touchée est celle des 50-60 ans, tout sexe confondu.

#Douleurs thoraciques en fonction de la maladie cardiaque
